...
compute_mfcc() {
    ...
    ##    EXEC="wav2mfcc $NUMCOEFS $NUMFILTERS 8 spk_8mu/sr_test/$filename.wav $w/$FEAT/$NAME/$filename.$FEAT"
    ##    echo $EXEC && $EXEC || exit 1
    ##done
    ## Final test verif
...
for cmd in $*; do
   ...
   #  printf $fh "nerr=%d\tntot=%d\terror_rate=%.2f%%\n", ($err, $ok+$err, 100*$err/($ok+$err));
       ...
       FOLDER=$3
       echo $FOLDER
       gmm_train  -v 1 -T 0.0005 -N40 -m $NGMMTRAIN -d $w/$FEAT/$FOLDER -e $FEAT -g $w/gmm/$FEAT/$FOLDER/world.gmm $lists/verif/users_and_others.train || exit 11
	   ...
	   #   * <code> gmm_verify ... | tee $w/verif_${FEAT}_${name_exp}.log </code>
        ...
        FOLDER=$2
        echo $FOLDER
        gmm_verify -d $w/$FEAT/$FOLDER -e mfcc -D $w/gmm/$FEAT/$FOLDER -w world -E gmm $lists/gmm.list $lists/verif/all.test $lists/verif/all.test.candidates | tee $w/verif_${FEAT}_${name_exp}.log
       ...
       (gmm_classify -d $w/mfcc/final -e mfcc -D $w/gmm/mfcc/2420 -E gmm $lists/gmm.list  $lists/final/class.test | tee $w/class_test.log) || exit 1
       ...
       gmm_verify -d $w/mfcc/finalverif -e mfcc -D $w/gmm/mfcc/2016 -w world -E gmm $lists/gmm.list $lists/final/verif.test $lists/final/verif.test.candidates | tee $w/verif_test.log
